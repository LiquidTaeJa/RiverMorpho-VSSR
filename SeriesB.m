...
Phin=linspace(-3,6,10);
Phi=(Phin(1:9)+Phin(2:10))/2;
D=2.^Phi/1000;
ng=size(D,2);
% [<0.25,<0.5,<1,<2,<4,<8,<16,<32,<64]/100
% fgravel=[0,0,0,20,15,5,5,25,15]/100;
% fsand=[0,0,15,0,0,0,0,0,0]/100;
% qgfT=1;
% qsfT=1;
% qtf=qgfT*fgravel+qsfT*fsand;
qtf=[18,17,13,3,11,16,16,5,1]/100;
qtf_unit=qtf;
qtfT=sum(qtf,2);
fsupply=qtf./qtfT;
fbase=fsupply;
...
% peak_time_index=round(num_morpho_steps*peak_time/1.5);
% sediment_feed_end_index=round(num_morpho_steps*1.5/1.5);
% qtfT=zeros(num_morpho_steps,1);
